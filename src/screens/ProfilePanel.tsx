...
export function ProfilePanel(props: ProfilePanelProps) {
  ...
  const handleSave = () => {
    updateProfile({ name: name.trim(), role: role.trim(), email: email.trim() });
    setEditing(false);
  };
    ...
    <>
      ...
      <header className="fixed top-0 right-0 left-0 md:left-[240px] h-16 border-b border-outline-variant bg-surface flex items-center justify-between px-margin-mobile md:px-margin-desktop w-full md:w-[calc(100%-240px)] z-20">
        ...
        <div className="flex items-center gap-md">
          <div className="flex items-center gap-sm">
            <button className="p-2 text-on-surface-variant hover:bg-surface-container-high rounded-full transition-colors focus-within:ring-2 focus-within:ring-primary-container cursor-pointer" aria-label="Notifications">
              <span className="material-symbols-outlined">notifications</span>
            </button>
            <button className="p-2 text-on-surface-variant hover:bg-surface-container-high rounded-full transition-colors focus-within:ring-2 focus-within:ring-primary-container cursor-pointer" aria-label="Help">
              <span className="material-symbols-outlined">help_outline</span>
            </button>
          </div>
          <img alt="Administrator Profile" className="w-8 h-8 rounded-full border border-outline-variant object-cover cursor-pointer" onClick={() => setScreen('profile')} src="https://lh3.googleusercontent.com/aida-public/AB6AXuCbQh4xsJO3aUayj1kFPLwtnqo4WCPfqOoiEwgCPvocUliwT_UVoSv5kHIVNdlnnTyB3oZcPZ_4RT8MgOMZKZ9h3wrd7otnFog-x6y6eDQ7j90cgMuA_eMfkhuGjRvN8su2jvAkRQ6-dsiJQW3doJeuGWQcywRKnLwMHAwM5YqgBLgH5G_nLe-3xHRMaisn5peXfIqlRr33GFnZyIUPHWTyCsaaKMRXyJakOhzBf3zerTc3STS1brP6O4t1SVrwFZ4YbiBpa_JZNtQ" />
        </div>
      </header>
      {/* SideNavBar */}
      <nav className="hidden md:flex fixed left-0 top-0 h-full w-[240px] border-r border-outline-variant bg-surface-container flex-col py-xl z-30">
        ...
        <ul className="flex flex-col gap-sm px-sm flex-grow">
          <li>{navLink('dashboard', 'Dashboard', 'dashboard')}</li>
          <li>{navLink('operations', 'Operations', 'assignment_add')}</li>
          <li>{navLink('insights', 'Insights', 'analytics')}</li>
          <li>{navLink('settings', 'Settings', 'settings')}</li>
          <li>{navLink('profile', 'Profile', 'account_circle', true)}</li>
        </ul>
        <div className="px-lg mt-auto">
          <img alt="OpsLoop Corporate Logo" className="w-12 h-12 object-contain opacity-50 grayscale" src="https://lh3.googleusercontent.com/aida-public/AB6AXuA7DIqbslKsiXDP51pGLUjaShd-WxW_ja4EJEUFSRpS3G2yrv6givdvXhZMc7cfMZNUN3HDmkwkq15MIQng2CT36KarviBLtNPNQIT06owlBTATHmw8fJlNNVZcrboFn-O7_Rn9WLVZSX6FK6jA5esNvVyOJFN3EC-AYeGupkNolHnuyXijjKFXlqS6IXOXvhHP5hOjkWnQFG2gRyERZqHb7AITeN-Wb6Snb7qmd7ImkPUvzGbBxeyTWf-7wqxm9DL5b6MSuF1Aph4" />
        </div>
      </nav>
      {/* Main Content Canvas */}
        ...
        <div className="w-full max-w-md bg-surface border border-outline-variant rounded-xl overflow-hidden flex flex-col h-[calc(100vh-120px)]">
          ...
          <div className="p-lg border-b border-outline-variant flex items-start gap-lg bg-surface-container-low">
            <div className="relative">
              <img alt="Administrator Profile Large" className="w-20 h-20 rounded-full border-2 border-outline-variant object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuDi9Bz72xzGbps1PVzHQEYl9XtSws1X3C7Gbm3GsSRtRfTJxaibu1EufpxWTaOLbvkuOmeJqaQru22GN_us_O42ytKlnSRDvsythCbAFAFwyj_bEzaPbSpe1kobyFrzBCJBT8QEetQ-6tLIjT9Eknzri0fgJtl7qt0AFZD4QOyF_DhJRWrDzvyfJli5ZyJ6kiofy1VmEoC4jLDAT4D1jSTicC46lcy2-KXNIJj7hQi30YNJw3PFL3YwLis95TuO6BA0Fl0YdCcAae0" />
              <button
                onClick={() => setEditing(!editing)}
                className="absolute bottom-0 right-0 bg-surface-container-highest border border-outline-variant rounded-full p-1 text-on-surface hover:text-primary transition-colors cursor-pointer"
                aria-label="Edit profile"
              >
                <span className="material-symbols-outlined" style={{fontSize: "16px"}}>{editing ? 'check' : 'edit'}</span>
              </button>
            </div>
            <div className="flex flex-col gap-xs pt-xs">
              {editing ? (
                <>
                  <input
                    className="font-headline-md text-headline-md text-on-surface bg-transparent border-b border-outline-variant outline-none w-full"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    aria-label="Name"
                  />
                  <input
                    className="font-body-sm text-body-sm text-on-surface-variant bg-transparent border-b border-outline-variant outline-none w-full"
                    value={role}
                  ...
                  <button
                    onClick={handleSave}
                    className="mt-2 px-3 py-1 bg-primary-container text-on-primary-container rounded-lg font-body-sm cursor-pointer self-start"
                  >
                    Save
                  </button>
                </>
                ...
                <>
                  <h2 className="font-headline-md text-headline-md text-on-surface">{state.profile.name}</h2>
            ...
            <section className="flex flex-col gap-md">
              ...
              <div className="flex flex-col gap-sm">
                ...
                <div className="flex items-center justify-between p-sm hover:bg-surface-container-low rounded-lg transition-colors">
                  ...
                  <button
                    onClick={() => updateProfile({ twoFactorEnabled: !state.profile.twoFactorEnabled })}
                    className={`w-10 h-6 rounded-full relative cursor-pointer border border-[#334155] transition-colors ${
                      state.profile.twoFactorEnabled ? 'bg-primary-container' : 'bg-surface-container-highest'
                    }`}
                    aria-label="Toggle two-factor authentication"
                  >
                    <div className={`w-4 h-4 rounded-full absolute top-1 transition-all ${
                      state.profile.twoFactorEnabled ? 'right-1 bg-on-primary-container' : 'left-1 bg-on-surface-variant'
                    }`} />
                  </button>
                </div>
                {/* Toggle Item 2 */}
                <div className="flex items-center justify-between p-sm hover:bg-surface-container-low rounded-lg transition-colors">
                  ...
                  <button
                    onClick={() => updateProfile({ emailNotifications: !state.profile.emailNotifications })}
                    className={`w-10 h-6 rounded-full relative cursor-pointer border border-[#334155] transition-colors ${
                      state.profile.emailNotifications ? 'bg-primary-container' : 'bg-surface-container-highest'
                    }`}
                    aria-label="Toggle email notifications"
                  >
                    <div className={`w-4 h-4 rounded-full absolute top-1 transition-all ${
                      state.profile.emailNotifications ? 'right-1 bg-on-primary-container' : 'left-1 bg-on-surface-variant'
                    }`} />
                  </button>
                </div>
                {/* Toggle Item 3 */}
                <div className="flex items-center justify-between p-sm hover:bg-surface-container-low rounded-lg transition-colors">
                  ...
                  <button
                    onClick={() => updateProfile({ betaFeatures: !state.profile.betaFeatures })}
                    className={`w-10 h-6 rounded-full relative cursor-pointer border border-[#334155] transition-colors ${
                      state.profile.betaFeatures ? 'bg-primary-container' : 'bg-surface-container-highest'
                    }`}
                    aria-label="Toggle beta features"
                  >
                    <div className={`w-4 h-4 rounded-full absolute top-1 transition-all ${
                      state.profile.betaFeatures ? 'right-1 bg-on-primary-container' : 'left-1 bg-on-surface-variant'
                    }`} />
                  </button>
                </div>
              </div>
          ...
          <div className="p-lg border-t border-outline-variant bg-surface-container-low mt-auto">
            <button
              ...
              className="w-full flex items-center justify-center gap-sm py-3 px-4 border border-outline-variant rounded-lg font-label-caps text-label-caps text-error hover:bg-surface-container-highest transition-colors focus-within:ring-2 focus-within:ring-primary-container focus-within:ring-offset-2 focus-within:ring-offset-background cursor-pointer"
            >
